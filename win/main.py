...
from tkinterweb import HtmlFrame # Новый виджет для GPU графики
...
class App(ctk.CTk):
    def __init__(self):
        ...
        # Виджет для GPU карты во вкладке 3D
        self.gpu_frame = HtmlFrame(self.t3d, messages_enabled=False)
        ...
        self.btn = ctk.CTkButton(self, text=strings.BTN_RUN, height=60, fg_color=styles.COLORS["dark"]["success"], font=styles.FONTS["title"], command=self.on_click, corner_radius=12)
    ...
    def on_upd(self, v, data): self.update_data = data; self.btn.configure(text=f"ОБНОВЛЕНИЕ v{v}", fg_color="#007acc")
    ...
    def run(self):
        try:
            ...
            if self.matrix is not None:
                ...
                # 3D Карта (GPU WebGL)
                html_content = viz.get_plotly_html(self.matrix, bx, by, gx, gy)
                self.gpu_frame.load_html(html_content)
                
                self.tabs.set(strings.TAB_2D)
                logic.save_settings({"host": self.ip.get(), "port": self.port.get(), "user": self.user.get(), "password": self.pwd.get(), "path": self.path.get(), "bed_x": self.bx.get(), "bed_y": self.by.get(), "grid_x": self.gx.get(), "grid_y": self.gy.get(), "z_sys": self.z_m.get(), "pitch": self.p_m.get()})
