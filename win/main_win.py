...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.set_app_icon(); self.center_window()
        self.last_raw_mutable = ""; self.last_raw_config = ""; self.matrix = None; self.config_editor = None 
    ...
    def toggle_log_view(self, state):
        ...
        if state:
            ...
            self.center_block.hide_raw_tab(); self.center_block.hide_config_editor_tab()

    def on_visualize_click(self):
        self.update_settings_from_ui(); storage_win.save_settings(self.settings)
    ...
    def worker_fetch_data(self):
        ...
        try:
            transport = transport_win.SSHTransport(h, p, u, pw)
            ...
            if config_params: self.after(0, lambda: self.center_block.fill_config_fields(config_params))
            ...
            logger_win.error(f"Worker error: {e}"); self.after(0, lambda: self.right_panel.set_status("error"))
    ...
    def on_save_config_request(self):
        ...
        else: messagebox.showerror("Ошибка", "Не удалось сохранить конфигурацию.")
    ...
    def on_restore_backup_request(self, filename):
        ...
        if messagebox.askyesno("Подтверждение", f"Восстановить {filename}?"):
            if backup_win.restore_backup(h, p, u, pw, p_cfg, filename):
                messagebox.showinfo("Готово", "Восстановлено!"); data = self.config_editor.get_config_parameters(); self.center_block.fill_config_fields(data)

    def update_settings_from_ui(self):
        self.settings.update({"host": self.left_panel.ip.get(), "port": self.left_panel.port.get(), "user": self.left_panel.user.get(), "password": self.left_panel.pwd.get(), "path": self.left_panel.p_mesh.get(), "path_cfg": self.left_panel.p_cfg.get(), "grid_x": self.left_panel.gx.get(), "grid_y": self.left_panel.gy.get(), "bed_x": self.left_panel.bx.get(), "bed_y": self.left_panel.by.get()})
    ...
    def center_window(self):
        sw, sh = self.winfo_screenwidth(), self.winfo_screenheight(); w, h = int(sw * 0.9), int(sh * 0.85); self.geometry(f"{w}x{h}+{(sw-w)//2}+{(sh-h)//2}"); self.minsize(1350, 800)
    def on_closing(self): plt.close('all'); self.quit(); self.destroy(); os._exit(0)
    def set_app_icon(self):
        icon_path = logic_win.resource_path("icon.ico")
        if os.path.exists(icon_path):
            try:
                myappid = f'mycompany.myproduct.{logic_win.VERSION}'
                ctypes.windll.shell32.SetCurrentProcessExplicitAppUserModelID(myappid); self.iconbitmap(icon_path)
            except: pass
