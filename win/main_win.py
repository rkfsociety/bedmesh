...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.title(f"{strings_win.APP_TITLE} v0.084-win") # Обновил версию
        self.center_window()
        self.protocol("WM_DELETE_WINDOW", self.on_closing)
        
        self.matrix = None
        self.recs_data = [] # Для хранения данных отчета
        ...
        ctk.CTkLabel(self.sidebar, text=strings_win.SECTION_SSH, font=styles_win.FONTS["title"]).pack(pady=(20, 10))
        self.ip = ui_elements_win.LabeledEntry(self.sidebar, strings_win.LBL_IP, self.settings.get("host", "192.168.1.100"))
        ...
        self.pwd = ui_elements_win.LabeledEntry(self.sidebar, strings_win.LBL_PASS, self.settings.get("password", "raspberry"), show="*")
        self.path = ui_elements_win.LabeledEntry(self.sidebar, strings_win.LBL_PATH, self.settings.get("path", "/home/pi/printer_data/config/printer_mutable.cfg"))
        ctk.CTkButton(self.sidebar, text=strings_win.BTN_FETCH, command=self.fetch, corner_radius=8).pack(pady=10, padx=20)
        
        ctk.CTkLabel(self.sidebar, text=strings_win.SECTION_GEOMETRY, font=styles_win.FONTS["ui_bold"]).pack(pady=(20, 5))
        ...
        # Кнопка копирования
        self.copy_btn = ctk.CTkButton(self.right, text="📋 КОПИРОВАТЬ ОТЧЕТ", font=styles_win.FONTS["micro"], height=30, fg_color="#333333", command=self.copy_to_clipboard)
        self.copy_btn.pack(pady=10, padx=20, fill="x")
        ...
        updater_win.check_for_updates("0.084-win", self.show_update_notify)
    ...
    def copy_to_clipboard(self):
        ...
        report = "--- Bed Mesh Alignment Report ---\n"
        for i in self.recs_data:
            report += f"{i['name']}: {i['val']:.3f} mm ({i['turns']}) -> {i['dir']}\n"
        self.clipboard_clear()
        self.clipboard_append(report)
        messagebox.showinfo("Clipboard", "Отчет скопирован в буфер обмена!")
    ...
    def fetch(self):
        try:
            res = logic_win.fetch_ssh(self.ip.get(), self.port.get(), self.user.get(), self.pwd.get(), self.path.get())
    ...
    def run(self):
        try:
            ...
            if self.matrix is not None:
                ...
                viz_win.draw_2d_map(self.t2d, self.matrix, bx, by, gx, gy)
                self.tabs.set(strings_win.TAB_2D)
                logic_win.save_settings({"host": self.ip.get(), "port": self.port.get(), "user": self.user.get(), "password": self.pwd.get(), "path": self.path.get(), "bed_x": self.bx.get(), "bed_y": self.by.get(), "grid_x": self.gx.get(), "grid_y": self.gy.get(), "z_sys": self.z_m.get(), "pitch": self.p_m.get()})
