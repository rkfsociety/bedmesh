...
import logic_win, styles_win, ui_elements_win, strings_win, updater_win, backup_win, stats_win, recs_win, map2d_win
...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.matrix, self.recs_data, self.update_data, self.cfg_content = None, [], None, ""
        ...
        # Инициализация полей (чтобы избежать AttributeError)
    ...
    def fetch(self):
        try:
            m = logic_win.fetch_ssh(self.ip.get(), self.port.get(), self.user.get(), self.pwd.get(), self.p_mesh.get())
            self.cfg_content = logic_win.fetch_ssh(self.ip.get(), self.port.get(), self.user.get(), self.pwd.get(), self.p_cfg.get())
            ...
            if self.cfg_content:
                ...
                self.bed_t.entry.delete(0, 'end'); self.bed_t.entry.insert(0, logic_win.get_cfg_value(self.cfg_content, "leviQ3", "bed_temp"))
            self.run()
    ...
    def save_cfg(self):
        ...
        text = self.cfg_content
        text = logic_win.set_cfg_value(text, "cs1237", "sensitivity", self.s1.get())
        text = logic_win.set_cfg_value(text, "cs1237", "head_block_sensitivity", self.s2.get())
        text = logic_win.set_cfg_value(text, "cs1237", "scratch_sensitivity", self.s3.get())
        text = logic_win.set_cfg_value(text, "leviQ3", "bed_temp", self.bed_t.get())
        
        try:
            logic_win.save_ssh(self.ip.get(), self.port.get(), self.user.get(), self.pwd.get(), self.p_cfg.get(), text)
    ...
    def run(self):
        try:
            ...
            if self.matrix is not None:
                ...
                logic_win.save_settings({"host": self.ip.get(), "port": self.port.get(), "user": self.user.get(), "password": self.pwd.get(), "path": self.p_mesh.get(), "path_cfg": self.p_cfg.get(), "bed_x": self.bx.get(), "bed_y": self.by.get(), "grid_x": self.gx.get(), "grid_y": self.gy.get(), "z_sys": self.z_m.get(), "pitch": self.p_m.get()})
    ...
    def refresh_recs(self, _=None):
        if self.matrix is not None:
            self.recs_data, is_s = recs_win.get_recs(self.matrix, self.z_m.get(), float(self.p_m.get()), int(self.gx.get()))
