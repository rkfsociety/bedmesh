...
class App(ctk.CTk):
    def __init__(self):
        ...
        # Инициализируем редактор (транспорт передадим позже при подключении)
    ...
    def toggle_log_view(self, state):
        ...
        if state:
            ...
            # Если данные для подключения есть, пробуем обновить бэкапы и параметры сразу
            # Это решает проблему "невидимых бэкапов" при запуске
            h = self.left_panel.ip.get()
            if h and h != "0.0.0.0":
                threading.Thread(target=self._quick_update_backups_and_config, daemon=True).start()
    ...
    def _quick_update_backups_and_config(self):
        """Фоновое обновление списка бэкапов и текущих параметров без полной визуализации"""
        try:
            h, p, u, pw = self.settings["host"], self.settings["port"], self.settings["user"], self.settings["password"]
            p_cfg = self.settings["path_cfg"]
            
            # Создаем транспорт если его еще нет
            ...
            # Подтягиваем параметры в поля
            data = self.config_editor.get_config_parameters()
            if data:
                self.after(0, lambda: self.center_block.fill_config_fields(data))
                
        except Exception as e:
            logger_win.error(f"Фоновое обновление не удалось: {e}")
    ...
    def on_save_config_request(self):
        ...
        if self.config_editor.save_config(new_params):
            ...
            # Сразу обновляем список бэкапов после сохранения
            threading.Thread(target=self._quick_update_backups_and_config, daemon=True).start()
        else:
