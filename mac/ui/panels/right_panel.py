...
class RightPanel(QWidget):
    ...
    def set_update_available(self, release_data: dict, latest_tag: str | None = None, current_version: str | None = None):
        ...
        if latest.lower().startswith("v"):
            latest = latest[1:]
        if current_version:
            self.lbl_version_status.setText(
                f"Требуется обновление (v{current_version} → v{latest})" if latest else "Требуется обновление"
            )
        else:
    ...
    def _create_correction_card(self, title, val_mm, turns, direction_text):
        ...
        lbl_turns.setStyleSheet(
            f"font-size: 12px; font-weight: bold; color: {'#4ade80' if 'ВВЕРХ' in direction_text else '#f87171'}"
        )
