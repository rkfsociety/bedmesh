...
st.title("📏 Bed Mesh Visualizer v4.5")
...
# Настройки по умолчанию (на основе твоего файла)
default_vals = {
    "grid_x": 10, "grid_y": 10,
    ...
    try:
        ...
        if raw_content.strip().startswith('{'):
            ...
            if mesh_data:
                ...
                st.sidebar.success("✅ Файл прочитан")
...
if st.button("ПОСТРОИТЬ И АНАЛИЗИРОВАТЬ"):
    if data_input:
        ...
        if len(nums) >= (gx * gy):
            ...
            st.error(f"Нужно {gx*gy} точек, найдено {len(nums)}")
...
if st.session_state.analyzed:
    ...
    with col1:
        st.markdown("### 📊 3D Рельеф")
        fig3 = go.Figure(data=[go.Surface(
            x=np.linspace(mx_min, mx_max, gx), 
            y=np.linspace(my_min, my_max, gy), 
            z=display_matrix, 
            colorscale='RdYlBu_r'
        )])
        fig3.update_layout(
            scene=dict(xaxis_range=[0, bed_x], yaxis_range=[0, bed_y], aspectratio=dict(x=1, y=1, z=0.4)),
    ...
    with col2:
        ...
        # Шрифт мельче для сетки 10х10
        f_size = 7 if gx > 7 else 9
        for i in range(gy):
            for j in range(gx):
                val = display_matrix[i, j]
                txt = ax.text(xc[j], yc[i], f"{val:.3f}", ha="center", va="center", fontweight='bold', fontsize=f_size)
    ...
    # ИНСТРУКЦИЯ ВНИЗУ
    st.divider()
    with st.expander("🔧 Мастер выравнивания (Инструкция)", expanded=True):
        mode = st.radio("Инструмент:", ["Винты", "Валы Z"], horizontal=True)
        
        if mode == "Винты":
            p = st.selectbox("Шаг", [0.7, 0.5, 0.8], format_func=lambda x: f"M4 (0.7мм)" if x==0.7 else f"M3 (0.5мм)")
            ...
            z_cnt = st.selectbox("Валов Z:", [2, 3, 4])
            pts = {}
            if z_cnt == 2: pts = {"Левый": np.mean(display_matrix[:,0]), "Правый": np.mean(display_matrix[:,-1])}
            elif z_cnt == 3:
                scheme = st.selectbox("Схема:", ["2 Спереди + 1 Сзади (центр)", "1 Спереди (центр) + 2 Сзади"])
                if "2 Спереди" in scheme: pts = {"Перед-Л": display_matrix[0,0], "Перед-П": display_matrix[0,-1], "Зад-Ц": display_matrix[-1, gx//2]}
                else: pts = {"Перед-Ц": display_matrix[0, gx//2], "Зад-Л": display_matrix[-1,0], "Зад-П": display_matrix[-1,-1]}
